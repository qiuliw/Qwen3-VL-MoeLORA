...
│   ├── multi_agent.py              # Planner / Manager / Knowledge / Responder 逻辑
│   ├── model_client.py             # 模型加载与图片输入处理
│   ├── rag_pipeline.py             # 知识库加载 + FAISS 检索
...
- **Knowledge Agent**：基于 `rag_pipeline.py` 构建的 FAISS 检索器，支持 `.txt/.md/.pdf` 多编码加载。
- **Responder Agent**：通过 `model_client.py` 调用本地 Qwen3-VL（支持 LoRA 权重 & 图文输入）。
- **Qt 前端**：`main_app.py` 复刻 `main_langchain_ui.py` 的交互体验，展示 Planner/Manager 面板、RAG 参考与最终答复

> 默认知识库目录：`multi_agent/knowledge_base/`，界面右下角可勾选 “启用知识库检索 (RAG)” 开关。
