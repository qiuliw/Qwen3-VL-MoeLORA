...
#         # 视觉模块（关键补充）
#         "visual_q_proj", "visual_k_proj", "visual_v_proj", "visual_o_proj",
#         "vision_encoder.layers.0.attention.qkv.proj",  # 视觉编码器注意力层
#         "vision_encoder.layers.0.attention.proj"
#     ],
#     inference_mode=False,
#     r=16,  # 8G显存建议r=16（原64可能显存不足）
#     lora_alpha=32,
#     lora_dropout=0.05,
#     bias="none",
# )
...
val_config = LoraConfig(
    task_type=TaskType.CAUSAL_LM,
    # 必须包含训练时新增的视觉模块，与训练配置完全一致
    target_modules=[
        # 文本模块
        "q_proj", "k_proj", "v_proj", "o_proj",
        # 视觉模块（与训练时的target_modules保持一致）
        "visual_q_proj", "visual_k_proj"],
    inference_mode=True,  # 验证/推理模式（关闭梯度计算）
    r=8,  # 与训练时的r保持一致（必须相同）
    lora_alpha=16,  # 与训练时保持一致
    lora_dropout=0.05,  # 与训练时保持一致
    bias="none",
)

# 获取测试模型
val_peft_model = PeftModel.from_pretrained(model, model_id="./output/Qwen3-VL-4Blora/checkpoint-62", config=val_config)
